...
public struct BackyardViewport: View
{
  ...
  public var body: some View
  {
    BackyardViewportLayout(birdNaturalScale: bird?.species?.naturalScale ?? 1)
    {
      ...
      ForEach(leadingPlantPlacements)
      { placement in
      ...
      ForEach(trailingPlantPlacements)
      { placement in
...
// MARK: - BackyardViewportContentModifier
...
extension View
{
  fileprivate func backyardViewportContent(_ value: BackyardViewportContent) -> some View
  {
...
{
  private func parallaxOffsetInScrollView(_ length: Double = 20) -> Double
  {
