...
@Model public class Backyard
{
  ...
  public init(name: String?)
  {
    ...
    waterRefillDate = .now
    foodRefillDate = .now
    presentingVisitor = false
    isFavorite = false
    timeIntervalOffset = TimeInterval(hours: 12)
    floorVariant = 0
    fountainVariant = 0
    leadingSilhouetteVariant = 0
    trailingSilhouetteVariant = 0
    leadingForegroundPlantVariant = 0
    trailingForegroundPlantVariant = 0
  }
...
extension Backyard
{
  public func snapshots(through date: Date, total: Int = 12) -> [BackyardSnapshot]
  {
    ...
    let relevantVisitorEvents = visitorEvents.filter
    { event in
