...
extension ClosedRange where Bound: BinaryFloatingPoint
{
  ...
  public func value(percent: Bound, clamped: Bool = true) -> Bound
  {
    var percent = percent
    if clamped
    {
  ...
  public func percent(for value: Bound, clamped: Bool = true) -> Bound
  {
...
extension BinaryFloatingPoint
{
  ...
  public func easeInOut(clamped: Bool = true) -> Self
  {
    assert(isFinite)
  ...
  public func symmetricEaseInOut(clamped: Bool = true) -> Self
  {
    assert(isFinite)
    let timing = clamped ? min(max(self, 0), 1) : self
    if timing <= 0.5
  ...
  public func percent(truncation: Self) -> Self
  {
    assert(isFinite)
  ...
  public func map(to range: ClosedRange<Self>, clamped: Bool = true) -> Self
  {
    assert(isFinite)
  ...
  public func remap(from originalRange: ClosedRange<Self>, to newRange: ClosedRange<Self>, clamped: Bool = true) -> Self
  {
    assert(isFinite)
    return newRange.value(percent: originalRange.percent(for: self, clamped: clamped), clamped: clamped)
  }

  public func interpolate(to destination: Self, percent: Self, clamped: Bool = true) -> Self
  {
...
extension Angle
{
  ...
  {
    let max = Double.pi * 2
    let deltaAngle = (destination.radians - radians).truncatingRemainder(dividingBy: max)
    let distance = (2 * deltaAngle).truncatingRemainder(dividingBy: max) - deltaAngle
    return Angle.radians(radians + distance * percent)
  }
}
